%This version was modified by Jaco Verster on 27-10-2016, modified parts clearly
%marked in code. Modification allows a Nx3 de Bruin sequence to be
%generated using 7 colour combinations. This is then later used to generate
%a de Bruijn pattern image.

function sequence = debruijn_sequence(num_chars,subsequence_length)
...
if floor(num_chars)~=num_chars | num_chars<1
    disp('Error: Number of characters must be positive integer')
    sequence = [];
    return
end

if floor(subsequence_length)~=subsequence_length | subsequence_length<1
    disp('Error: Subsequence length must be positive integer')
    sequence = [];
    return
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [combination_list{subsequence_length:-1:1}] = ndgrid(1:6); %%%%Edit: "6" used to replace "num_chars" in the ndgrid function, to allow for 6 colors used.
    ...
    %%%%Edit: above line comented out to attampt to reconstruct a combination list
    %without duplicate values. Will work only for subsequence_length = 3!!!
    
    %%%Edit: new code created:
    combination_list_temp = reshape(cat(subsequence_length+1,combination_list{:}),[],subsequence_length);
    combination_list_new = zeros(length(combination_list_temp),3);
    a=0;
    for iter = 1:length(combination_list_temp)
        if combination_list_temp(iter,1) == combination_list_temp(iter,2)
            combination_list_new(iter,:) = [0 0 0];
            a = a+1;
        elseif combination_list_temp(iter,2) == combination_list_temp(iter,3)
            combination_list_new(iter,:) = [0 0 0];
            a = a+1;
        else
            combination_list_new(iter,:) = combination_list_temp(iter,:);
        end
    end
    
    %finally generate the new combination list
    combination_list = zeros(length(combination_list_temp)-a,3);
    
    count = 1;
    for iter2 = 1:length(combination_list_temp)
        if combination_list_new(iter2,1) > 0
            combination_list(count,:) = combination_list_new(iter2,:);
            count = count+1;
        end
    end
end
